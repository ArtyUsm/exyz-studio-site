...
(() => {
  ...
  function save(attempt) {
    ...
    Promise.resolve(w(STATE_FILE, payload))
      ...
      .catch(() => {
        saving = false;
        // A write failure (transient host/network issue) must not silently
        // drop the drop — retry with backoff instead of giving up after one
        // attempt, up to a few tries.
        if (attempt < 4) {
          setTimeout(() => save(attempt + 1), 300 * Math.pow(2, attempt));
        } else if (saveDirty) {
          saveDirty = false; save();
        }
      });
  }
